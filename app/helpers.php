...
if (!function_exists('productSalePrice')) {
    ...
    {
        $product = \App\FinishedProduct::find($id);

        if ($product) {
            return number_format((float)$product->sale_price, 2, '.', '');
        }
...
/**
 * Get Total Returned Quantity for a Raw Material from a Purchase
 */
if(!function_exists('getTotalReturnedQuantity')){
    function getTotalReturnedQuantity($rmId, $purchaseId = null){
        $query = \App\PurchaseReturnDetails::where('item_id', $rmId)
            ->where('del_status', 'Live')
        ...
        if($purchaseId) {
            $purchase = \App\RawMaterialPurchase::find($purchaseId);
            if($purchase) {
                $query->whereHas('purchaseReturn', function($q) use ($purchase) {
                    $q->where('pur_ref_no', $purchase->reference_no);
                });
            }
        }
        
        return $query->sum('return_quantity_amount') ?? 0;
    }
}

/**
 * Get Available Quantity for Return from a Purchase
 */
if(!function_exists('getAvailableReturnQuantity')){
    function getAvailableReturnQuantity($purchaseId, $rmId){
        $purchase = \App\RawMaterialPurchase::find($purchaseId);
        if(!$purchase || $purchase->status != 'Final') {
            return 0;
        }
        
        $purchaseItem = \App\RMPurchase_model::where('purchase_id', $purchaseId)
            ->where('rmaterials_id', $rmId)
            ->where('del_status', 'Live')
            ->first();
        ...
        $purchasedQty = $purchaseItem->quantity_amount;
        $returnedQty = getTotalReturnedQuantity($rmId, $purchaseId);
 ...
 */
if(!function_exists('canReturnPurchase')){
    function canReturnPurchase($purchaseId){
        $purchase = \App\RawMaterialPurchase::find($purchaseId);
        if(!$purchase || $purchase->del_status != 'Live') {
            return false;
        }
        // Only finalized purchases can be returned
        return $purchase->status == 'Final';
    }
}

/**
 * Generate Purchase Return Reference Number
 */
if(!function_exists('generatePurchaseReturnReference')){
    function generatePurchaseReturnReference(){
        $count = \App\PurchaseReturn::count();
        return "PR-" . str_pad($count + 1, 6, '0', STR_PAD_LEFT);
    }
}

/**
 * Get Total Sale Returned Quantity
 */
if(!function_exists('getTotalSaleReturnedQuantity')){
    function getTotalSaleReturnedQuantity($saleId, $productId, $saleDetailId = null){
        $query = \App\SaleReturnDetails::where('product_id', $productId)
        ...
        if($saleDetailId) {
            $query->where('sale_detail_id', $saleDetailId);
        }
        
        return $query->sum('product_quantity') ?? 0;
    }
}

/**
 * Get Available Sale Return Quantity
 */
if(!function_exists('getAvailableSaleReturnQuantity')){
    function getAvailableSaleReturnQuantity($saleId, $productId, $saleDetailId = null){
        $sale = \App\Sales::find($saleId);
        if(!$sale || $sale->status != 'Final') {
            return 0;
        }
        
        $saleDetail = null;
        if($saleDetailId) {
            $saleDetail = \App\SaleDetail::where('sale_id', $saleId)
        ...
        $soldQty = $saleDetail->product_quantity;
        $returnedQty = getTotalSaleReturnedQuantity($saleId, $productId, $saleDetailId);
...
/**
 * Generate Sale Return Reference Number
 */
if(!function_exists('generateSaleReturnReference')){
    function generateSaleReturnReference(){
        $count = \App\SaleReturn::where('del_status', 'Live')->count();
        return "SR-" . str_pad($count + 1, 6, '0', STR_PAD_LEFT);
    }
...
function getMenus()
{
    return \App\Menu::with('activities')
        ->where('status', 1)
        ->orderBy('order', 'asc')
        ->get();
}