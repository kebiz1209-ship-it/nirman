                    ...
                    <table id="datatable" class="table table-striped">
                        <thead>
                            <tr>
                                ...
                                <th class="width_10_p">@lang('index.return_date')</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($obj as $value)
                                <tr>
                                    ...
                                    <td>{{ getDateFormat($value->return_date) }}</td>
                                    <td>
                                        @if ($value->return_status != 'Final')
                                            @if (routePermission('sale-return.edit'))
                                                <a href="{{ url('sale-returns') }}/{{ encrypt_decrypt($value->id, 'encrypt') }}/edit"
                                                    class="button-success" data-bs-toggle="tooltip" data-bs-placement="top"
                                                    title="@lang('index.edit')"><i class="fa fa-edit"></i></a>
                                            @endif
                                        @endif
                                        @if (routePermission('sale-return.view-details'))
                                            <a href="{{ url('sale-returns') }}/{{ encrypt_decrypt($value->id, 'encrypt') }}"
                                        ...
                                        @if (routePermission('sale-return.download-invoice'))
                                            <a href="{{ route('sale_returns.download_return_invoice', encrypt_decrypt($value->id, 'encrypt')) }}"
                                                class="button-info" data-bs-toggle="tooltip" data-bs-placement="top"
                                                title="@lang('index.download')"><i class="fa fa-download"></i></a>
                                        @endif
                                        @if ($value->return_status != 'Final')
                                            @if (routePermission('sale-return.delete'))
                                                <a href="#" class="delete button-danger"
                                                    ...
                                                    <form action="{{ route('sale-returns.destroy', $value->id) }}"
